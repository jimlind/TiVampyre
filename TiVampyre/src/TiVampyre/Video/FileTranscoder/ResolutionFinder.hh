...
class ResolutionFinder
{
    ...
    public function findResolution(string $filePath): array
    {
        ...
        $resolution    = $this->parseResolution($processOutput);

        return $resolution;
    }

    protected function parseResolution(string $processOutput): array
    {
        $matches = [];
        $pattern = '|\+ size: (\d+)x(\d+),|';
